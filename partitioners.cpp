...
void non_neighbors_greedy(const Graph& graph,
                          int partitions_number,
                          Partition* partition)
{
  vector<int> nodes;
  vector<int> partition_score(partitions_number);
  ...
  for (vector<int>::iterator node = nodes.begin();
    ...
    //select partition with minimum non neighbors
    int selected_partition = -1;
    int nn = nodes.size() + 1;
    for (int i=0; i<partitions_number; i++)
    {
      int partition_nn = partition->get_partition_size(i) - partition_score[i];
      if (partition_nn < nn)
      {
        selected_partition = i;
        nn = partition_nn;
